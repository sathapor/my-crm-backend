        ...
        <div className="flex-1 glass p-6 md:p-8 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden">
          ...
          {activeTab === 'facebook_page' && (
            <div className="space-y-6 animate-fade-in">
              ...
              {!fbAppId && (
                <div className="p-5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-2xl space-y-4">
                  <div className="flex items-start gap-3">
                    <span className="text-2xl">🔑</span>
                    <div>
                      <p className="font-bold text-amber-800 dark:text-amber-300">ขั้นตอนแรก: ใส่ Facebook App ID</p>
                      <p className="text-xs text-amber-700 dark:text-amber-400 mt-1">คุณต้องสร้าง App บน <strong>developers.facebook.com</strong> และเพิ่ม Messenger ก่อน จากนั้นคัดลอก App ID มาใส่ที่นี่</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 p-3 bg-white dark:bg-gray-900 border border-amber-300 dark:border-amber-700 rounded-xl focus:ring-2 focus:ring-amber-400 focus:outline-none font-mono text-sm"
                    />
                    <button onClick={saveAppId} className="px-4 py-3 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl transition text-sm">
                      บันทึก
                    </button>
                  </div>
                  <p className="text-[11px] text-gray-400">
                    ยังไม่มี App?{' '}
                    <a href="https://developers.facebook.com/apps/create" target="_blank" rel="noopener noreferrer" className="text-[#1877F2] underline">คลิกสร้าง App ที่นี่</a>
                    {' '}→ เลือก Other → Business → ตั้งชื่อ → เพิ่มผลิตภัณฑ์ Messenger
                  </p>
                </div>
              )}
