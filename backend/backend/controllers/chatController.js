...
exports.facebookWebhookVerify = async (req, res) => {
  ...
  try {
    ...
    // รองรับรหัสกลางที่เราแปะไป หรือรหัสที่เก็บใน DB
    const expectedToken = fbAcc.verify_token || 'crm_facebook_verify_token_2024';
    ...
    console.error('facebookWebhookVerify error:', err.message);
    // หากเกิด Error ให้ลองยอมรับรหัสกลางไปก่อนเพื่อความสะดวก
    if (token === 'crm_facebook_verify_token_2024') {
      console.log('✅ Facebook Webhook verified using fallback token');
      return res.status(200).send(challenge);
    }
